...
public class DispatcherServlet extends HttpServlet{
	...
	public void init(ServletConfig servletConfig) throws ServletException {//项目启动的时候运行
		...
		UploadHelper.init(servletContext);//设置文件上传得一些属性
	}
	// 要访问servlet的时候 执行
	@Override
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		ServletHelper.init(request,response);//封装Servlet API 在ServletHelper之中
		try{
		//获取请求方法与请求路径
		String requestMethod=request.getMethod().toLowerCase();//get  post
		String requestPath=request.getPathInfo();
		if(requestPath.equals("/favicon.ico")){//网站图标则返回
			return;
		}
		//获取Action处理器
		...
		if(handler!=null){
			...
			/* 封装在了RequestHelper.createParam()方法之中  //创建请求参数对象
			...
			ServletHelper.destory();//移除 ServletHelper持有得ServletHelper对象
		}
